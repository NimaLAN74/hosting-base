...
echo "=== E2E Stock Monitoring (routes + API) ==="
...
echo "3. GET /api/v1/stock-monitoring/health (expect 200)..."
HEALTH_CODE=$(curl -sS -o /dev/null -w "%{http_code}" "$BASE_URL/api/v1/stock-monitoring/health" 2>/dev/null)
...
if [ $FAILED -eq 1 ]; then
  echo "=== E2E Stock Monitoring: FAILED ==="
  exit 1
fi
echo "=== E2E Stock Monitoring: PASSED ==="
exit 0
